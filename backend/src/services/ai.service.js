...
async function processImages(imagePaths) {
  ...
  try {
    ...
    const prompt = `
      Analyze these images of a clothing item. The images include tags, measurements (yardstick), and a weight/inventory shot.
      ...
      4. MEASUREMENTS_NOTE: Read the size tags (including international sizes like AU/US/EU if shown) and examine the yardstick/ruler in the photos to estimate key measurements (like waist and inseam). Format exactly like this: "Tagged size [list all tag sizes]. Approximate waist appears to measure about [X] inches across laying flat based on photo. Approximate inseam appears to be about [Y] inches based on photo. Please compare measurements to a similar item you own before purchasing."
